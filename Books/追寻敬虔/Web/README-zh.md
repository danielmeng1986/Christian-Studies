...
当前 MVP 已包含第五章的确定性构建、章节菜单预留、三栏阅读布局、窄屏侧栏、明亮／暗色／护眼三种主题，以及左栏脚注交互。笔记编辑界面将在下一阶段实现。

本地服务现已包含阶段 3 的章节笔记数据层：`GET /api/chapters/05/notes` 读取纳入 Git 的 JSON 数据源，带修订号的 `PUT` 请求负责原子保存。下一阶段的笔记编辑界面才会开始调用这些接口。
