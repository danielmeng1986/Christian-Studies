...
第五章现有两个已批准的正式阅读版本。不带版本的
`http://127.0.0.1:4173/chapters/05/` 仍打开默认的 Word 转写版；现代简体
润译版位于 `http://127.0.0.1:4173/editions/chatgpt-zh-cn/chapters/05/`。
可使用顶部“版本”选择器切换。笔记、讨论、正文修订和 AI 上下文 Manifest
均按版本隔离；请求尚未发布或本章不可用的版本时，会回到默认版并显示说明。

第五章双版本审核试点位于
`http://127.0.0.1:4173/review/editions/chatgpt-zh-cn/chapters/05/`。页面把现有
Word 转写版与现代简体润译版逐段并列显示；每个对应段落可依次标记为 `draft`、
`reviewed`、`approved`。只有全部段落都获批准后，“批准整章”才会启用。每段可
追加修改意见；待处理意见必须先标记为已处理，该段和整章才能获批准。浏览器中的
两份对照正文继续保持只读。审核写入沿用本地回环来源限制、会话写入令牌、原子
替换和旧版本冲突保护。
英文原稿通过同源的运行时内存代理显示在可收起的参考栏中；PDF 从获许可的来源
网址读取，但不会持久化到仓库、审核记录或阅读器构建产物。

本次版本发布同时修改了生成页面和本地服务。应用这次更新后，请先执行构建，
并把已经运行的后端重启一次。以后若只修改正文而服务代码没有变化，重新构建并
刷新网页即可。

当前阅读版会确定性构建全部 20 章，并包含章节导航、带当前分节与阅读进度反馈的本章目录、三栏阅读布局、窄屏侧栏、三种阅读主题、脚注与经文交互、逐章笔记、“与 AI 讨论”和本地资料库。本章目录从顶部工具栏打开，与当前阅读现场的脚注／经文参照保持分离。
