...
2. Run `scripts/convert_qfg_chapter.py` against `Books/追寻敬虔/Original/QFG Draft 20170911 20211005.doc`, first writing to temporary output paths.
3. Compare temporary output with the existing chapter. Preserve intentional repository metadata while preferring Word text, paragraph structure, and semantic styles.
4. Verify all headings, quotations, inline italics, footnote references, footnote bodies, and chapter boundaries.
5. Write the approved chapter and `References/Footnotes-NN.md` only after the audit passes.
6. Run the script again with `--audit-only`, then run the link/content checks in the canonical specification.
